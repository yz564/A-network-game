...
public class ActionCostCalculator {
     ...
     *
     * @return HashMap where key is int of tech level and values is cummalative tech costs required
     *     to upgrage that tech level
     */
    ...
    public HashMap<String, Integer> calculateUpgradeSpyUnitCost(
            ActionInfo action, WorldMap worldmap) {
        if (action.getUpgradeUnitActionInfo() != null) {
            HashMap<String, Integer> costs = new HashMap<String, Integer>();
            int totalNumUnits = action.getTotalNumUnits();
    ...
    public HashMap<String, Integer> calculateResearchPatentCost(
            ActionInfo action, WorldMap worldmap) {
        if (action.getResearchPatentActionInfo() != null) {
            HashMap<String, Integer> costs = new HashMap<>();
            int targetTerritoryNum = action.getTargetTerritoryNames().size();
            costs.put("tech", targetTerritoryNum * 3);
            return costs;
        }
        return null;
    }

    public HashMap<String, Integer> calculateResearchCloakingCost(
            ActionInfo action, WorldMap worldmap) {
        if (action != null) {
            HashMap<String, Integer> costs = new HashMap<String, Integer>();
            costs.put("tech", 100);
