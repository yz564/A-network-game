...
public class V2Territory extends AbstractTerritory {
     ...
     * @param troops are the troops that are present in this territory. HashMap key is troop name,
     *     and value is the Troop objects representing troops in this territory.
    ...
    public V2Territory(
            ...
            Integer artsResearchRate,
            Integer businessResearchRate,
            Integer engineeringResearchRate,
            Integer medicalResearchRate,
            Integer sportsResearchRate,
            int size) {
    ...
    public HashMap<String, Integer> getPatentResearchRate() {
        return this.patentResearchRate;
    }

    @Override
    public String getDomain() {
        String domain = "arts";
        for (String field : patentResearchRate.keySet()) {
            if (patentResearchRate.get(domain) < patentResearchRate.get(field)) {
                domain = field;
            }
        }
        return domain;
    ...
    /**
     * Helper function for findAllMinMoveCost() Find the territory with the minimum cost that has
     ...
     */
    private String findMinCostTerritory(
            HashSet<String> visited, HashMap<String, Integer> moveCost) {
        ...
        while (visited.size() < reachable.keySet().size()) {
            ...
            for (String neighborName : neighbors.keySet()) {
                if (!visited.contains(neighborName)) {
                    int newCost =
                            moveCost.get(currName) + curr.getMoveCost(neighbors.get(neighborName));
