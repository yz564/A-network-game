...
/**
 * client: the socket connected with the client name: the player's name isEnd to
 * chech is the player lose in and out are ObjectIOstream ready is important to
 * control the player's thread, basic logic is writeObject once and then must
 * readObject once...because of the blocking issue availableUnitNum: the total
 * unit number a player have at the beginning of placement watch: a status
 * indicates whether sending message to clients or not tmp: used to store the
 * most recently ObjectIO read in the player's thread.
 */
