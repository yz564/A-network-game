...
public class ServerOrderHelper {
     ...
     *
     * @param map the WorldMap to update status with.
    ...
    /**
     * Deducts the cloaking turns for each territory, should be called after each turn.
     *
     * @param map the WorldMap to Deduct the cloaking turns on.
     */
    public void deductCloakingTurns(WorldMap map) {
        for (String territoryName : map.getMyTerritories()) {
            if (map.getTerritory(territoryName).getCloakingTurns() > 0) {
                map.getTerritory(territoryName).reduceCloakingTurns();
            }
        }
    }

    /**
     * Does the manipulations on map after each turn's actions are resolved.
     *
     * @param map the WorldMap to do the manipulations on.
     * @param playerNames the list of players that are participated in one game.
     */
    public void doAfterTurn(WorldMap map, ArrayList<String> playerNames) {
        sendCreditToPlayers(map, playerNames);
        updateMultiVizStatus(map, playerNames);
        deductCloakingTurns(map);
    }
}
