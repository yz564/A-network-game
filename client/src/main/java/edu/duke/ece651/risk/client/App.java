...
public class App implements Runnable, GUIEventListener {
    ...
    public void runOnePlayer() throws Exception {
        String tmpS;
        checkIn();
        // if Player.wait is true, the System.in read at here. Otherwise, the System.in read in
        // player
        // thread
        ...
        while (true) {
            if (players.get(currentRoomId).isWait()) {
                players.get(currentRoomId).ready =
                        true; // once arrive here (let the main thread listen instead of let the
                // player thread
                // listen), set player.ready = true, tell the player to set player.wait=false.
                if ((tmpS = stdIn.readLine())
                        != null) { // arrive here only when player.wait is true
                    if (tmpS.equals(
                            "/leave")) { // if request leave, tell the server and go back join room
                        // page
                        requestLeave();
                        break;
                    }
                    players.get(currentRoomId)
                            .updateInput(
                                    tmpS); // if not request leave, pass what the client get from
                    // system.in to the
                    // player.tmpS
                }
            }
