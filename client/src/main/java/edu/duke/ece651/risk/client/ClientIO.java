...
public interface ClientIO {
     ...
     * their current territory to some other own territory. "A" if player wants to
     * Attack enemy territory. "D" if player is Done and does not want to either
     * Move or Attack.
     *
     * @returns "M", "A" or "D" depending on players choice.
     */
    String readActionName(String playerName, String prompt);

    /*
     * Read a territory name from a player.
     *
     * @param prompt is the text message that a user sees asking to input territory
     * name.
     *
     * @returns territory name as a String.
     */
    String readTerritoryName(String prompt);
     ...
     */
    int readNumUnits(String prompt);

    /*
     * Print a textual represntation of the RISK map.
     *
     * @param view computes the string that represents RISK map.
     *
     * @param worldMap is the object that represents RISK map.
     *
     * @param playerNames is the list of players playing the game.
     */
    void printMap(MapTextView view, WorldMap worldMap, ArrayList<String> playerNames);
}
