...
public class ClientOrderHelper {
   ...
   *
   * @param totalUnitNum   is an int that represents the total number of unit to
   *                       place on territories.
   * @param territoryNames is an ArrayList of String represents all territories'
    ...
    for (String territoryName : territoryNames) {
      while (true) {
        ...
        if (toPlace > totalUnitNum) {
          out.println("Invalid input: You have  only <" + totalUnitNum + "> unit(s) to place, but you want to place <"
              + toPlace + "> units.");
